...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index+1]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
                ...
                <span class="nav-link"  data-images-toggle="${value}">${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag")){let color=$(this)
color.attr("style","background-color: none;");return}
$(".active-tag").removeClass("active active-tag");$(this).addClass("active-tag");var tag=$(this).data("images-toggle")
if($(this).hasClass("active-tag")){let mes_tag=$(".nav-link")
for(let i=0;i<mes_tag.length;i++){$(mes_tag[i]).attr("style","background-color: none;")}
let color=$(this)
